...
A new `argocd app health-check` command has been added to provide comprehensive health checks for applications, including sync status, resource health, pod readiness, service connectivity, and resource drift detection.
...
- `--interval`: Health check interval in seconds for continuous mode (default: 10).
- `-o`, `--output`: Output format. One of: `json|yaml|wide|name` (default: `wide`).
